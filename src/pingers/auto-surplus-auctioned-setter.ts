...
export class AutoSurplusAuctionedSetter {
  protected adjuster: adminContracts.AuctionedSurplusSetter
  ...
  constructor(
    ...
    this.adjuster = this.transactor.getGebContract(
      adminContracts.AuctionedSurplusSetter,
      setterAddress
    )
  }
  ...
  public async ping() {
    ...
    try {
      tx = this.adjuster.recomputeSurplusAmountAuctioned(rewardReceiver)
      await this.transactor.ethCall(tx)
    } catch (err) {
      if (err.startsWith('AuctionedSurplusSetter/wait-more')) {
        console.log('AuctionedSurplusSetter, wait more')
      } else {
